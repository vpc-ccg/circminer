...
void Alignment::global_banded_alignment_drop(char *s, int n, char *t, int m, int w, int &on_s, int &on_t) {
    ...
    for (k = 2; k <= m + n; ++k) {
        ...
        for (i = lb[0]; i <= ub[0]; ++i) {
            ...
            dpx[i][j] = maxM3(
                    dpx[i - 1][j - 1] + score_ch[static_cast<uint8_t> (s[i - 1])][static_cast<uint8_t> (t[j - 1])],
...
void Alignment::global_banded_alignment(char *s, int n, char *t, int m, int w) {
    ...
    for (j = 1; j <= w; j++) {
        for (i = 1; i <= j + w; i++) {
            dp[i][j] = minM3(
                    dp[i - 1][j - 1] + diff_ch[static_cast<uint8_t> (s[i - 1])][static_cast<uint8_t> (t[j - 1])],
                    dp[i - 1][j] + 1,
                    dp[i][j - 1] + 1);
        }
    }
...
void Alignment::global_banded_alignment_reverse(char *s, int n, char *t, int m, int w) {
    ...
    for (j = 1; j <= w; j++) {
        for (i = 1; i <= j + w; i++) {
            dp[i][j] = minM3(
                    dp[i - 1][j - 1] + diff_ch[static_cast<uint8_t> (s[n - i])][static_cast<uint8_t> (t[m - j])],
                    dp[i - 1][j] + 1,
                    dp[i][j - 1] + 1);
        }
    }
