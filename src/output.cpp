...
void SAMOutput::write_pam_rec_pe(Record *rec1, Record *rec2) {
    ...
    if (mr->type == CONCRD or mr->type == DISCRD or mr->type == CHIORF or mr->type == CHIBSJ or mr->type == CHI2BSJ or
        ...
        fprintf(outsam, "%s\t%s\t%u\t%u\t%d\t%u\t%u\t%c\t%d\t%s\t%u\t%u\t%d\t%u\t%u\t%c\t%d\t%d\t%d\t%d\t%d\n",
                rec1->rname, mr->chr_r1.c_str(), mr->spos_r1, mr->epos_r1, mr->mlen_r1, mr->qspos_r1, mr->qepos_r1,
                r1_dir, mr->ed_r1, mr->chr_r2.c_str(), mr->spos_r2, mr->epos_r2, mr->mlen_r2, mr->qspos_r2,
                mr->qepos_r2, r2_dir, mr->ed_r2, mr->tlen, mr->junc_num, mr->gm_compatible, mr->type);
    } else {
        fprintf(outsam, "%s\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t*\t%d\n", rec1->rname,
                mr->type);
    }
