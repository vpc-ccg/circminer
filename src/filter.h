...
public:
    ...
    FilterRead(char *save_fname, bool pe, int round, bool first_round, bool last_round, char *fq_file1, char *fq_file2,
               const vector <ContigLen> &chr_info);
    ...
    int process_read(int thid, Record *current_record1, Record *current_record2, int kmer_size,
    ...
    int process_mates(int thid, const chain_list &forward_chain, const Record *forward_rec,
                      const chain_list &backward_chain, const Record *backward_rec,
    ...
    void pair_chains(const chain_list &forward_chain, const chain_list &reverse_chain, vector <MatePair> &mate_pairs,
